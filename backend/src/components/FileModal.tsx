...
export default function FileModal({ invoice, readOnly = false, onClose, onFilesChanged }: Props) {
            ...
            {invoice.files.length === 0 ? (
              ...
              invoice.files.map((f) => (
                <div key={f._id} className="file-item">
                  ...
                  {!readOnly && <button className="file-remove" onClick={() => handleDelete(f._id)} title="Remove">✕</button>}
                </div>
              ))
            )}
          </div>
