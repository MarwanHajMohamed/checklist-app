...
const ChecklistInstance_1 = __importDefault(require("../models/ChecklistInstance"));
const auth_1 = require("../middleware/auth");
const router = (0, express_1.Router)();
router.use(auth_1.requireAuth);
router.get('/', async (_req, res) => {
    const items = await ChecklistInstance_1.default.find().sort({ createdAt: -1 });
    res.json(items);
});
router.post('/', async (req, res) => {
    const { templateId, invoiceNum } = req.body;
    const item = await ChecklistInstance_1.default.create({ templateId, invoiceNum, doneItems: [] });
