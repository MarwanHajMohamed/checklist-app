...
router.get("/", async (req, res) => {
  ...
  const items = await SendListItem_1.default
    .find({ date })
...
router.patch("/reorder", async (req, res) => {
  ...
  await Promise.all(
    order.map(({ id }, idx) =>
      SendListItem_1.default.findByIdAndUpdate(id, { order: idx }),
    ),
  );
  res.json({ ok: true });
});
...
router.patch("/:id", async (req, res) => {
  ...
  const item = await SendListItem_1.default.findByIdAndUpdate(
    req.params.id,
    update,
    { new: true },
  );
